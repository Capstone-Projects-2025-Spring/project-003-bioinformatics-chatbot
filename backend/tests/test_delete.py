...
def test_delete_valid_document(client, app):
    ...
    with app.app_context():
        ...
        embedding_exists_after = db.session.execute(
            select(vector_db.EmbeddingStore).where(vector_db.EmbeddingStore.id.like(f"%{uploaded_doc.document_name}.{uploaded_doc.document_type}%"))
        ).fetchone()
...
def test_delete_database_error(client, app, monkeypatch):
    ...
    with app.app_context():
        monkeypatch.setattr(db.session, "commit", faulty_commit)  # Force DB commit to fail
